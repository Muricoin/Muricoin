...
Muricoin Core 1.8.1 is primarily a bugfix release, bringing Muricoin Core in
line with Bitcoin 0.9.3. Muricoin Core 1.8.1 also adds in support for printing
...
* Disable Transifex based translation process (not used by Muricoin Core)
* Add checkpoints around AuxPoW switchover block (371,337)
* Disable connections from pre-1.8 Muricoin Core clients
* Use HTTPS with murchain.info
...
There is a known issue with quantities in JSON used by the RPC interface (https://github.com/muricoin/muricoin/issues/134).
...
This only affects values significantly below 1 Mur, however merchants may wish to round to
the nearest Mur on all transactions for now.
