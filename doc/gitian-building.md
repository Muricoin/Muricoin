...
*Setup instructions for a gitian build of Muricoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Muricoin
...
- [Building Muricoin](#building-muricoin)
...
Clone the git repositories for muricoin and gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/muricoin/muricoin
```
...
Follow the instructions in [doc/release-process.md](release-process.md) in the muricoin repository
...
To build Muricoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the muricoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/muricoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/muricoin/muricoin
...
For example:
```bash
URL=https://github.com/laanwj/muricoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit muricoin=${COMMIT} --url muricoin=${URL} ../muricoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit muricoin=${COMMIT} --url muricoin=${URL} ../muricoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit muricoin=${COMMIT} --url muricoin=${URL} ../muricoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/muricoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/muricoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/muricoin-osx-build.assert
```
...
[muricoin/gitian.sigs](https://github.com/muricoin/gitian.sigs/) repository, or if that's not possible create a pull request. 
